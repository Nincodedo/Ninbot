...
public class StatsCommand extends AbstractCommand {
    ...
    private Message displayServerStats(MessageReceivedEvent event) {
        final java.util.Map<java.lang.String, java.util.List<dev.nincodedo.ninbot.components.stats.Stat>> statMap =
                statManager
                        .getStatMapByServerId(event.getGuild().getId());
        EmbedBuilder embedBuilder = new EmbedBuilder();
        for (String key : statMap.keySet()) {
            final java.util.List<dev.nincodedo.ninbot.components.stats.Stat> statList = statMap.get(key)
                    .stream()
                    .sorted(Comparator.comparing(Stat::getCategory))
                    .limit(5)
                    .collect(Collectors.toList());
            embedBuilder.addField(WordUtils.capitalizeFully(key), "", false);
            for (final dev.nincodedo.ninbot.components.stats.Stat stat : statList) {
    ...
    private Message displayRoleStats(Guild server) {
        ...
        Map<Role, Integer> roleMap = server.getMembers()
                .stream()
        ...
        roleStatList.stream()
                .limit(limit)
                .forEach(roleStat -> embedBuilder.appendDescription(roleStat.name + ": " + roleStat.amount + "\n"));
        return new MessageBuilder(embedBuilder).build();
    }
