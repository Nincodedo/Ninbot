...
public class TempVoiceChannelManager extends StatAwareListenerAdapter {
    ...
    private static final org.apache.logging.log4j.Logger log =
            org.apache.logging.log4j.LogManager.getLogger(TempVoiceChannelManager.class);
    ...
    private void checkIfShouldCreateTempChannel(Guild guild, Member member, VoiceChannel channelJoined) {
        ...
        log.trace("hasPermission: {}, channel join {} is temp creator: {}", hasPermission(guild,
                ...
                .getName(), channelJoined.getName().startsWith(Emojis.PLUS));
        if (hasPermission(guild, Permission.MANAGE_CHANNEL) && channelJoined.getName().startsWith(Emojis.PLUS)) {
    ...
    private void createTemporaryChannel(VoiceChannel channelJoined, Guild guild, Member member) {
        String channelNameType = channelJoined.getName().substring(2);
        String channelName = String.format("%s's %s", member.getEffectiveName()
                .replace(Emojis.PLUS, ""), channelNameType);
        ...
        createVoiceChannel(guild, channelJoined, channelName).queue(voiceChannel -> {
            ...
            guild.moveVoiceMember(member, voiceChannel).queue(aVoid -> {
                final int position = channelJoined.getPosition();
                modifyVoiceChannelPositions(guild, channelJoined).selectPosition(voiceChannel)
                        .moveTo(position + 1)
    ...
    private RestAction<VoiceChannel> createVoiceChannel(Guild guild, VoiceChannel joinedChannel, String format) {
    ...
    private OrderAction<GuildChannel, ChannelOrderAction> modifyVoiceChannelPositions(Guild guild,
            VoiceChannel joinedChannel) {
        if (joinedChannel.getParent() != null) {
    ...
    private void deleteTemporaryChannel(VoiceChannel voiceChannel) {
        voiceChannel.delete()
                .queueAfter(10, TimeUnit.SECONDS, success -> repository.findByVoiceChannelId(voiceChannel.getId())
