...
public class PathogenCommand extends AbstractCommand {
    ...
    private Message getInfectionServerStats(Guild guild) {
        ...
        final java.util.List<net.dv8tion.jda.api.entities.Role> roleList =
                guild.getRolesByName(PathogenConfig.getROLE_NAME(), true);
        if (!roleList.isEmpty()) {
            final java.util.List<net.dv8tion.jda.api.entities.Member> users = guild.getMembersWithRoles(roleList);
            List<String> userIds = users.stream().map(ISnowflake::getId).collect(Collectors.toList());
            final java.util.List<dev.nincodedo.ninbot.components.fun.pathogen.user.PathogenUser> infectedUsers =
                    pathogenUserRepository
                            .getAllByUserIdIsIn(userIds);
            ...
            infectedUsers.stream()
                    ...
                    .ifPresent(pathogenUser -> embedBuilder.addField(resourceBundle.getString("command.pathogen.stats"
                            + ".max"), String
                            .valueOf(pathogenUser.getInfectionLevel()), false));
        } else {
    ...
    private List<String> getUserInfectionLevel(MessageReceivedEvent event) {
        ...
        final dev.nincodedo.ninbot.components.fun.pathogen.user.PathogenUser pathogenUser =
                pathogenUserRepository.getByUserIdAndServerId(event
                        .getAuthor()
