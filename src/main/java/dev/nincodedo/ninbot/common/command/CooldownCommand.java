...
public abstract class CooldownCommand extends AbstractCommand {
    ...
    protected MessageReceivedEventMessageExecutor executeCommand(MessageReceivedEvent event) {
        ...
        if (lastTimeCommandUsed != null && Instant.now()
            ...
            return new MessageReceivedEventMessageExecutor(event).addReaction(Emojis.COOLDOWN_5_CLOCK);
        } else {
    ...
    protected abstract MessageReceivedEventMessageExecutor executeCooldownCommand(MessageReceivedEvent event);
}
